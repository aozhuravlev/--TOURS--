...
### Последние исправления (2026-01-21):
- topics.json синхронизирован с папками в media/photos (12 категорий, 85 подтем)
- Scheduler генерирует story series (было: одиночные stories)
- Publisher подключён к Instagram через media_uploader + SCP
- Очищена история контента от ошибочных записей
...
### Рефакторинг модерации (приоритет)
Изменить workflow модерации:
- Отправлять фото + текст (вместо готовых видео)
- Кнопки: Approve / Edit / Delete для каждой истории
- Рендеринг видео только после одобрения
- Публикация на следующее утро
