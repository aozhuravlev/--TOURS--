...
- Генерация Stories с 9 визуальными эффектами (zoom, pan, static)
- Генерация Posts
...
- **9 эффектов** вместо единственного Ken Burns:
  ...
  - static (без движения)
- **~20% сторизок — статика** (STATIC_PROBABILITY = 0.2)
- Каждая сторизка в серии получает случайный эффект независимо
- `--static` флаг отключает все эффекты
- Обратная совместимость: JSON persistence поддерживает и старый `ken_burns`, и новый `motion_effects`
